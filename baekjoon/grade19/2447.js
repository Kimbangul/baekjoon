...
const makeStar = (input) => {
  if ( input <= 1 ) return '*';

  const pattern = makeStar(input / 3);
  const line = mergeLine(new Array(3).fill(pattern));
  const middlePattern = new Array(input / 3).fill(` `.repeat(input / 3)).join('\n');

  const middleLine = mergeLine([pattern, middlePattern, pattern]);

  return `${line}\n${middleLine}\n${line}`;
}

const result = makeStar(input);
console.log(result);
