...
const isPalindrome = (str, front, rear) => {
  counter++;
  if ( front >= rear ) return 1;
  else if ( str[front] !== str[rear] ) return 0;
  else return isPalindrome(str, front+1, rear-1);
};

for ( let i = 0; i < input.length; i++ ) {
  counter = 0;
  result += `${isPalindrome(input[i], 0, input[i].length - 1)} ${counter}\n`;
}
