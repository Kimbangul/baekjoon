...
const sliceString = (strLength) => {
  if ( strLength <= 1 ) return "-";
  const sliceLength = Math.floor(strLength / 3);
  const mid = ' '.repeat(sliceLength);

  return sliceString(sliceLength) + mid + sliceString(sliceLength);
}


for ( let i = 0; i < input.length; i++ ) {
  let strLength = Math.pow(3, input[i]);

  result += sliceString(strLength) + '\n';
}
