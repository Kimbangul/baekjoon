...
const search = (arr, searchCnt) => {
  const lastPickedNum = arr[arr.length - 1] || 0;

  if ( maxNum - lastPickedNum < searchCnt ) return;
  ...
  for ( let i = lastPickedNum + 1; i <= maxNum; i++ ) {
    search([...arr, i], searchCnt - 1);
  }
};
